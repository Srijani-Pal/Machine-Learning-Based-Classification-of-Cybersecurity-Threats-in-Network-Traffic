...
Uses sampling for large datasets to reduce training time
...
datasets = {
    'CIC-IDS-2017': ('normalized_cleaned_cic_ids_merged.csv', 100000),  # Sample 100k
    'DDOS': ('normalized_cleaned_ddos_merged.csv', None),  # Use all (single class - skip)
    'UNSW': ('normalized_cleaned_unsw_merged.csv', None)   # Use all
}
...
for dataset_name, dataset_info in datasets.items():
    filename, sample_size = dataset_info
    print("\n" + "=" * 70)
    ...
    try:
        ...
        if sample_size:
            ...
            print("[OK] Loaded sample: " + str(sample_size) + " rows from dataset")
        else:
            df = pd.read_csv(filepath)
            print("[OK] Loaded dataset with shape: " + str(df.shape))
        
        print("[OK] Dataset shape: " + str(df.shape))
